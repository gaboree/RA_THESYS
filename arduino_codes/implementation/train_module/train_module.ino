...
void loop() {
	...
	while(network.available()){
		...
		if (master_header.from_node == 00){
			if (receivedData[0] !=  previous_speed){
	...
	//===== Sending =====//
	// acknowledge received data to central module 1 - command executed 0 - command not executed
	RF24NetworkHeader train_header(master_node);
	network.write(train_header, &ack_speed, sizeof(ack_speed));
