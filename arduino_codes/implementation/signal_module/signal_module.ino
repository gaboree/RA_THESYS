...
void loop() {
  ...
  while (network.available()) {
    ...
    network.read( master_header, &incomming_data, sizeof(incomming_data));
    if (master_header.from_node == node_acc) {
      for (int i = 0; i < sizeof(incomming_data); i++)
        local_signal[i] = incomming_data[i];
    }
  }
